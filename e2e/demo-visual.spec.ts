...
test("PL-02–04: масштаб, якоря и панель в обоих режимах", async ({
  page,
}, info) => {
    ...
    for (const scale of [70, 140]) {
      ...
      for (const title of ["Перед началом работы", "Диагностика"]) {
            ...
            page.getByRole("heading", { name: title, exact: true }).evaluate(
              (node, state) => {
                ...
                const panel = state.mobile
                  ? document
                      .querySelector(".reading-sidebar-bar")!
                      .getBoundingClientRect().height
                  : 0;
                return (
                  rect.top >= header + panel &&
        ...
        if (mobile && mode === "fullscreen") {
          await openReadingTools(page);
          const tools = await page.locator(".reading-settings").boundingBox();
          const exit = await page
            .getByRole("button", { name: "Выйти из полноэкранного режима" })
            .boundingBox();
          expect(tools!.height).toBeGreaterThanOrEqual(44);
          expect(exit!.y).toBeGreaterThanOrEqual(0);
          expect(exit!.y + exit!.height).toBeLessThanOrEqual(
...
test("PL-07: поворот PDF резервирует полные габариты во всех масштабах", async ({
  page,
}, info) => {
  ...
  for (const zoom of [70, 100, 140]) {
    ...
    for (let rotation = 0; rotation < 360; rotation += 90) {
      ...
      const bounds = await page
        .getByTestId("file-preview-bounds")
        .boundingBox();
...
test("PL-09: домены строками, ошибки и заголовки таблиц", async ({
  page,
}, info) => {
  ...
  await dialog
    .getByRole("button", { name: "Добавить домен", exact: true })
    .click();
  await dialog
  ...
  await page
    ...
    .click();
  await expect(
    dialog.getByLabel("Рабочий домен 3", { exact: true }),
  ).toHaveValue("docs.demo1609.example");
  await dialog
    .getByLabel("Рабочий домен 3", { exact: true })
    .fill("SEVERPROM.RU");
  await dialog.getByRole("button", { name: "Сохранить компанию" }).click();
  ...
  await page
    .getByRole("button", { name: "Пояснение: Требовать заполнения" })
    .focus();
  await expect(page.getByRole("tooltip")).toBeVisible();
  await page.screenshot({
    animations: "disabled",
    path: "/tmp/demo-table-sticky-tooltip.png",
  });
});

...
const experimentUrl =
  "./?page=landing&role=guest&backgroundArchive=1&background=scatter";
...
const orbitInk = (page: Page) =>
  page.getByTestId("auth-reactive-canvas").evaluate((element) => {
    const canvas = element as HTMLCanvasElement;
    ...
    for (let i = 0; i < pixels.length; i += 4) {
      ...
      const y =
        Math.floor(i / 4 / canvas.width) / (canvas.height / bounds.height);
...
const prepareScene = async (page: Page) => {
  ...
  await expect(
    page.getByRole("button", { name: /07.*Разлёт/ }),
  ...
  await page
    .getByRole("button", { name: /01.*Тишина/ })
    .evaluate((button: HTMLButtonElement) => button.click());
  await expect(page.locator(".portal-auth-grid")).toBeVisible();
  await page.clock.install({ time: 0 });
  await page.clock.pauseAt(1000);
  await page
    .getByRole("button", { name: /07.*Разлёт/ })
    .evaluate((button: HTMLButtonElement) => button.click());
  await expect(page.getByTestId("auth-reactive-canvas")).toBeVisible();
  await page.waitForTimeout(100);
};

const sweepWord = async (page: Page, y = page.viewportSize()!.height / 2) => {
  const { width } = page.viewportSize()!;
  await page.mouse.move(width * 0.16, y);
  await page.clock.runFor(16);
  for (let step = 1; step <= 18; step += 1) {
    await page.mouse.move(width * (0.16 + (step / 18) * 0.68), y);
    await page.clock.runFor(16);
  }
};

test("эксперимент удерживает штрихи на орбите, разбрасывает и возвращает после ухода курсора", async ({
  page,
}) => {
  test.slow();
  ...
  // Sweep through the body of the official logo, above its descending «ф» stroke.
  const canvasScale = await page
    .getByTestId("auth-reactive-canvas")
    .evaluate(
      (element) =>
  ...
  await sweepWord(
    page,
    (rest.top + (rest.bottom - rest.top) * 0.7) / canvasScale,
  );
  ...
  await page
    .getByTestId("wordmark-landing")
    .getByRole("button", { name: "Войти", exact: true })
    .click();
  await expect(page).toHaveURL(/background=scatter/);
  await expect(page.getByLabel("Электронная почта")).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Настроить разлёт", exact: true }),
  ).toHaveCount(0);
  await expect(page.getByTestId("portal-auth-backdrop")).toHaveAttribute(
    "data-background",
    "scatter",
  );
  ...
  await expect(page.getByRole("group", { name: "Варианты фона" })).toHaveCount(
    0,
  );
});

test("эксперимент при reduced-motion показывает неподвижную надпись", async ({
  page,
}) => {
...
test("настройки разлёта применяются сразу, копируются и сохраняются", async ({
  page,
}) => {
  await page.emulateMedia({ colorScheme: "light" });
    ...
    Object.defineProperty(navigator, "clipboard", {
      value: {
        writeText: async (text: string) => {
          document.documentElement.dataset.copiedSettings = text;
        },
      },
    });
  });
  ...
  await page
    .getByRole("button", { name: "Настроить разлёт", exact: true })
    .click();
  const panel = page.getByRole("region", { name: "Настройки разлёта" });
  await expect(
    page.getByRole("button", { name: "Свернуть настройки" }),
  ).toBeFocused();
  await expect(panel.getByRole("slider")).toHaveCount(9);
  const preset = [
    ["Сила разлёта", "0.3"],
    ["Область разлёта", "0.55"],
    ["Тяга обратно", "20"],
    ["Торможение", "3.8"],
    ["Кручение палочек", "0.8"],
    ["Длина хвостика", "0"],
    ["Неравномерность разлёта", "1"],
    ["Расстояние от курсора", "60"],
    ["Удержание на круге", "0.6"],
  ];
  for (const [name, value] of preset)
    await expect(panel.getByRole("slider", { name, exact: true })).toHaveValue(
      value,
    );
  const force = panel.getByRole("slider", {
    name: "Сила разлёта",
    exact: true,
  });
  await panel
    .getByRole("slider", { name: "Удержание на круге", exact: true })
    .press("Home");
  ...
  await panel
    .getByRole("slider", { name: "Область разлёта", exact: true })
    .press("ArrowRight");
  await page.getByRole("button", { name: "Скопировать настройки" }).click();
  await expect(panel.getByRole("status")).toHaveText(
    "Настройки скопированы — отправь их в чат.",
  );
  const copied = JSON.parse(
    (await page.locator("html").getAttribute("data-copied-settings")) as string,
  );
  expect(copied).toMatchObject({
    experiment: "maxsoft-scatter-v2",
    settings: { force: 2.5, radius: 0.6 },
  });
  ...
  await expect(
    panel.getByRole("slider", { name: "Область разлёта", exact: true }),
  ).toHaveValue("0.6");
  await page.getByRole("button", { name: "Сбросить настройки" }).click();
  await expect(force).toHaveValue("0.3");
  await expect(
    panel.getByRole("slider", { name: "Удержание на круге", exact: true }),
  ).toHaveValue("0.6");
  await expect(
    panel.getByRole("slider", { name: "Область разлёта", exact: true }),
  ).toHaveValue("0.55");
...
test("если браузер запрещает копирование, настройки можно скопировать вручную", async ({
  page,
}) => {
  await page.addInitScript(() => {
    Object.defineProperty(navigator, "clipboard", {
  ...
  await expect(page.getByRole("alert")).toContainText(
    "Скопируй текст ниже вручную",
  );
  const text = page.getByRole("textbox", { name: "Настройки для отправки" });
  await expect(text).toBeVisible();
  expect(JSON.parse(await text.inputValue())).toMatchObject({
    experiment: "maxsoft-scatter-v2",
  });
});

test("ранее сохранённые параметры разлёта сохраняются при добавлении орбиты", async ({
  page,
}) => {
  await page.addInitScript(() =>
    localStorage.setItem(
      "maxsoft-scatter-settings-v1",
      JSON.stringify({
        force: 1.2,
        radius: 1.4,
        spring: 18,
  ...
  await page
    ...
    .click();
  await expect(
    page.getByRole("slider", { name: "Сила разлёта", exact: true }),
  ).toHaveValue("1.2");
  await expect(
    page.getByRole("slider", { name: "Область разлёта", exact: true }),
  ).toHaveValue("1.4");
  await expect(
    page.getByRole("slider", { name: "Расстояние от курсора", exact: true }),
  ).toHaveValue("60");
  await expect(
    page.getByRole("slider", { name: "Удержание на круге", exact: true }),
  ).toHaveValue("0.6");
});

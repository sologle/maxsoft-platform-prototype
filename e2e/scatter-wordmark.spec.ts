...
test("эксперимент разбрасывает надпись и возвращает её даже под неподвижным курсором", async ({ page }) => {
...
test("настройки разлёта применяются сразу, копируются и сохраняются", async ({ page }) => {
  ...
  await panel.getByRole("slider", { name: "Радиус влияния", exact: true }).press("ArrowRight");
  ...
  expect(copied).toMatchObject({ experiment: "maxsoft-scatter-v1", settings: { force: 2.5, radius: 1.05 } });
  ...
  await expect(panel.getByRole("slider", { name: "Радиус влияния", exact: true })).toHaveValue("1.05");
  await page.getByRole("button", { name: "Сбросить настройки" }).click();
  await expect(force).toHaveValue("1");
  await expect(panel.getByRole("slider", { name: "Радиус влияния", exact: true })).toHaveValue("1");
  await page.goto("./");
  await expect(page.getByRole("button", { name: "Настроить разлёт", exact: true })).toHaveCount(0);
});
...
test("если браузер запрещает копирование, настройки можно скопировать вручную", async ({ page }) => {
  ...
  expect(JSON.parse(await text.inputValue())).toMatchObject({ experiment: "maxsoft-scatter-v1" });
});

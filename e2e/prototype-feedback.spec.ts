...
test("база знаний переключается между таблицей и карточками", async ({
  page,
}) => {
...
test("фирменный выпадающий список открывает стилизованное меню", async ({
  page,
}) => {
  await page.goto("./?page=knowledge&role=portal-admin");
  await page
  ...
  await expect(
    page.getByRole("button", {
      name: "Открыть варианты. Выбрано: По названию",
    }),
  ).toBeVisible();
});

test("администратор настраивает доступ компании к материалам", async ({
  page,
}) => {
  ...
  const access = page.getByRole("switch", {
    name: "Доступ: Настройка сетевой лицензии",
  });
  ...
  await page.goto(
    "./?page=article&resource=network-license&role=client-employee",
  );
  await expect(
...
test("поиск выделяет совпадение и открывает материал кликом по карточке", async ({
  page,
}) => {
  ...
  await expect(
    result
      .locator("mark")
      .filter({ hasText: /лиценз/i })
      .first(),
  ).toBeVisible();
  await expect(result.locator("xpath=ancestor::article")).toContainText("Совпадение в заголовке статьи");
  await result.click();
  await expect(
    page.getByRole("heading", { name: "Настройка сетевой лицензии" }),
  ).toBeVisible();
});

...
test("список управляется клавиатурой и возвращает фокус", async ({ page }) => {
  ...
  const trigger = page
    ...
    .locator("..")
    .getByRole("button", { name: /^Открыть варианты\. Выбрано:/ });
  await trigger.focus();
  ...
  await expect(trigger).toBeFocused();
  await expect(page.getByLabel("Сортировка", { exact: true })).toHaveAttribute(
    "tabindex",
    "-1",
  );
});
...
test("список экранов помещается над панелью сценариев", async ({ page }) => {
  ...
  await page
    .getByRole("button", { name: "Открыть варианты. Выбрано: База знаний" })
    .click();
  ...
  await menu
    .getByRole("option", { name: "Доступ к материалам", exact: true })
    .click();
  await expect(
...
test("список в форме сохраняет выбор и Escape закрывает только меню", async ({
  page,
}) => {
  ...
  await expect(dialog.getByLabel("Группа", { exact: true })).toHaveValue(
    "topics",
  );
...
test("выбор в неконтролируемом списке попадает в данные формы", async ({
  page,
}) => {
  ...
  await dialog
    .getByLabel("Корпоративная почта")
    .fill("select.check@integrator-pro.ru");
  await dialog
    .getByRole("button", {
      name: "Открыть варианты. Выбрано: ООО «СеверПромБИМ»",
    })
    .click();
  await dialog
    .getByRole("option", { name: "АО «Интегратор Про»", exact: true })
    .click();
  await dialog
    .getByRole("button", {
      name: "Открыть варианты. Выбрано: Сотрудник клиента",
    })
    .click();
  await dialog
    .getByRole("option", { name: "Администратор клиента", exact: true })
    .click();
  await dialog.getByRole("button", { name: "Отправить приглашение" }).click();
  await page.reload();
  const user = page
    .getByRole("row")
    .filter({ hasText: "Проверка Списка" })
    .or(page.locator("article").filter({ hasText: "Проверка Списка" }))
    .filter({ visible: true });

...
test("регистрация в существующей компании не меняет её реквизиты", async ({
  page,
}) => {
  ...
  expect(
    state.companies.find((c: { id: string }) => c.id === original.id),
  ).toEqual({
    ...original,
...
test("локальный фильтр переносится в полный поиск и возвращается без потери текста", async ({
  ...
  await page
    .getByRole("textbox", { name: "Фильтр материалов" })
    .fill("адрес сервера");
  await page
    .getByRole("button", {
      name: "Поиск по тексту статей и файлов",
      exact: true,
    })
    .click();
  await expect(
  ...
  await page.goBack();
  await expect(
    page.getByRole("textbox", { name: "Фильтр материалов" }),
  ).toHaveValue("адрес сервера");
  await page
    .getByRole("button", { name: "Очистить фильтр материалов", exact: true })
    .click();
  await expect(
    page.getByRole("textbox", { name: "Фильтр материалов" }),
  ).toHaveValue("");
...
test("видеостатья использует масштаб и выход чтения в обоих режимах", async ({
  page,
}, info) => {
  await page.goto(

  ...
  return (
    <>
      <PageHeading
        ...
        actions={
          <div
            aria-label="Режим просмотра файлов"
            className="flex rounded-xl border border-[var(--ms-border)] bg-white p-1 shadow-sm"
            role="group"
          >
            <button
      ...
      {!visible.length ? (
        ...
        <div
          className="overflow-x-auto rounded-2xl border border-[var(--ms-border)] bg-white shadow-[var(--ms-card-shadow)] ms-table-scroll"
          data-testid="files-table-view"
        >
          <table className="w-full min-w-[780px] border-collapse text-left text-sm">
            <thead>
              <tr className="border-b border-[var(--ms-border)] text-xs uppercase tracking-[.08em] text-[var(--ms-muted)]">
                ...
                <th className="px-5 py-4 font-bold">Раздел</th>
        ...
        <div
          ...
          data-testid="files-card-view"
        >
          {visible.map((file) => (
            <article
              className="group flex min-w-0 flex-col rounded-2xl border border-[var(--ms-border)] bg-white p-5 shadow-[var(--ms-card-shadow)] transition hover:-translate-y-0.5 hover:border-[var(--ms-primary)] hover:shadow-[var(--ms-card-shadow-hover)]"
              key={file.name}
            >
              <button
                aria-label={`${canPreviewFile(file) ? "Просмотреть" : "Скачать"} файл: ${file.name}`}
                className="min-w-0 text-left focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--ms-primary)]"
                onClick={() => openFile(file)}
                type="button"
              >
                <FileTypeIcon type={file.type} large />
                <span className="mt-5 block break-words font-heading text-lg font-bold leading-snug">
                  {file.name}
                </span>
                <span className="mt-2 block text-sm text-[var(--ms-muted)]">
    ...
    <>
      <Breadcrumbs
        items={[
          ...
          ...(role === "portal-admin"
            ? [{ label: "Реестр файлов", onClick: () => onNavigate("files") }]
            : []),
          { label: file.name },
        ]}
      />
      <PageHeading
        ...
        backLabel="Назад"
        eyebrow={`Просмотр файла · ${file.type} · ${file.size}`}
        onBack={() => goBack(onNavigate, backPage)}
        subtitle={`${primarySection} · ${usesLabel(relatedArticles.length)} · Демонстрационный предпросмотр`}
        title={file.name}
      />
      <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
        <section className="min-w-0 overflow-hidden rounded-2xl border border-[var(--ms-border)] bg-[#dce3ea] shadow-[var(--ms-card-shadow)]">
          <div className="flex flex-wrap items-center gap-2 border-b border-[var(--ms-border)] bg-white p-2.5">
            ...
            <button
              ...
              onClick={() => {
                ...
                onNotice("Страница повёрнута на 90°.");
              }}
              type="button"
            >
          ...
          <div className="min-h-[65dvh] max-h-[75dvh] overflow-auto p-4 sm:p-8">
            <DocumentPreview zoom={zoom} rotation={rotation}>
              <p className="text-xs font-bold uppercase tracking-[.16em] text-[#1478bd]">
                MaxSoft · {primarySection.split(" / ")[0]}
              </p>
              <h2 className="mt-8 font-heading text-3xl font-bold first-letter:uppercase">
                {documentTitle}
              </h2>
              <p className="mt-4 text-sm text-slate-500">
                Демонстрационный документ · 15 сентября 2026
              </p>
              <div className="mt-10 space-y-7 text-[15px] leading-7">
                {fileContent[file.name].map((part) => (
                  <section key={part.title}>
                    <h3 className="text-lg font-bold">{part.title}</h3>
                    <p className="mt-2">{part.text}</p>
                  </section>
                ))}
              </div>
            </DocumentPreview>
        ...
        <aside className="space-y-4 xl:self-start">
          <section className="rounded-2xl border border-[var(--ms-border)] bg-white p-5 shadow-[var(--ms-card-shadow)]">
            <h2 className="font-heading text-lg font-bold">О файле</h2>
            <dl className="mt-4 space-y-4 text-sm">
              ...
              <div>
                ...
                <dd className="mt-1 font-semibold">Анна Смирнова</dd>
              </div>
              <div>
                <dt className="text-xs font-bold uppercase tracking-[.08em] text-slate-400">
                  Обновлён
                </dt>
                <dd className="mt-1 font-semibold">{file.updated}</dd>
              </div>
            </dl>
          </section>
          <Button

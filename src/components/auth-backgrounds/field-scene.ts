...
export const createLivingField = (width: number, height: number): DrawScene => {
  const spacing = Math.max(LIVING_SPACING, Math.sqrt(width * height / MAX_MARKS));
  ...
  const wordTargets = createWordTargets(width, height, points.length);
  let idleTime = 0;
  let wordAmount = 0;

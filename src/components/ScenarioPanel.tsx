  ...
  return (
    <>
      ...
      <MotionRegion
        open={open}
        role="region"
        aria-label="Панель сценариев"
        onKeyDown={(event) => {
          if (event.key === "Escape") close();
        }}
        className="fixed bottom-[max(1rem,env(safe-area-inset-bottom))] right-3 z-[70] w-[min(370px,calc(100vw-24px))] max-h-[calc(100dvh-2rem)] overflow-y-auto rounded-2xl border border-white/10 bg-[#172b42]/96 p-4 text-white shadow-[0_18px_60px_rgba(0,0,0,.34)] backdrop-blur-xl sm:right-4"
      >
        <div className="mb-3 rounded-xl border border-white/10 bg-white/[.06] px-3 py-2.5">
          <p className="text-[10px] font-bold uppercase tracking-[.14em] text-white/50">
            Текущий экран
          </p>
          <p className="mt-1 truncate text-sm font-bold text-white">
            {currentPage?.label ?? location.page}
          </p>
        </div>
        <div className="flex items-start gap-3">
          <div className="min-w-0 flex-1">
            <p className="text-[11px] font-bold uppercase tracking-[.14em] text-white/55">
              Демонстрация
            </p>
          ...
          <button
            ...
            className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg text-white/70 transition hover:bg-white/10 hover:text-white"
            onClick={close}
            type="button"
          >
